...
@objc class TouchesFingerSensor: NSObject, TouchSensor {
    ...
    func rawValue(for spriteObject: SpriteObject) -> Double {
        guard let touchPosition = getTouchManager()?.lastPositionInScene() else { return type(of: self).defaultRawValue }

        if spriteObject.spriteNode.isHidden {
            return 0
        }
        ...
        let objectX = spriteObject.spriteNode.catrobatPosition.x
